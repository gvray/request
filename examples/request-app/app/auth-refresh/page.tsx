...
import createStore from 'storetify'

// 使用 storetify 管理 token，支持过期时间
...
function getToken(): string | null {
  ...
  const value = createStore(TOKEN_KEY)
  return typeof value === 'string' ? value : null
}

function setToken(token: string) {
  // 设置 5 分钟过期时间（300 秒）
  createStore(TOKEN_KEY, token, 300)
}

// Refresh Token: 30 分钟过期 (1800 秒)
function getRefreshToken(): string | null {
  if (typeof window === 'undefined') return null
  const value = createStore(REFRESH_TOKEN_KEY)
  return typeof value === 'string' ? value : null
}

function setRefreshToken(token: string) {
  // 设置 30 分钟过期时间（1800 秒）
  createStore(REFRESH_TOKEN_KEY, token, 1800)
}

function clearTokens() {
  createStore(TOKEN_KEY, null)
  createStore(REFRESH_TOKEN_KEY, null)
}
    ...
    <main>
      ...
      <div className="mb-8 p-4 bg-gray-50 rounded-xl space-y-2">
        ...
        <div className="text-xs text-gray-500 mt-2 pt-2 border-t border-gray-200">
          💾 Using <code className="bg-gray-200 px-1 rounded">storetify</code> for token storage with automatic expiration
        </div>
      </div>
