...
class RequestInterceptorManager {
  ...
  async run(url: string, options: RequestOptions): Promise<{ url: string; options: RequestOptions }> {
    let currentUrl = url
    ...
    for (const h of this.handlers) {
      ...
      if (Array.isArray(h)) {
        ...
        try {
          const result = (await maybeTwoArgs(currentUrl || '', currentOptions)) as { url: string; options: RequestOptions }
...
export class UniRequestClient<T = any> {
  ...
  useResponseInterceptor(i: InterceptorPair<UnifiedResponse<any>> | ((res: UnifiedResponse<any>) => Promise<UnifiedResponse<any>> | UnifiedResponse<any>)) {
  ...
  async request<U = any>(url: string, opts: RequestOptionsWithResponse): Promise<UnifiedResponse<U>>
  ...
  async request<U = any>(url: string, opts: RequestOptions = { method: 'GET' }): Promise<U | UnifiedResponse<U>> {
...
export const createClient = <T = any>(options: CreateClientOptions<T> = {}) => new UniRequestClient<T>(options)